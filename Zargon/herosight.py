...
class HeroSight():
    def __init__(self, screen):
        ...
        self.maskPos = {"x":self.gamevars.boardPos['x'],"y":self.gamevars.boardPos['y']}
        self.maskCenter = self.gamevars.unit * 3
        # Area Mask
        self.revealImage = pygame.image.load('images/Mask-7x7.png').convert_alpha()
        self.revealArea = pygame.Surface((self.gamevars.unit * 7, self.gamevars.unit * 7))
    ...
    def DrawSight(self, players, showMask=False):
        if showMask:
            self.revealMask = pygame.Surface(self.gamevars.screenSize, flags=pygame.SRCALPHA)
        ...
        for i, player in enumerate(players):
            pygame.draw.rect(self.screen, self.heroColors[i], pygame.Rect(player.grid[0]+self.maskPos["x"],player.grid[1]+self.maskPos["y"],self.gamevars.unit,self.gamevars.unit), 5)

        t = pygwidgets.DisplayText(self.screen, loc=(100, 100), value=str(len(self.heroTrail)), fontName='prozalibreextrabold', fontSize=128, width=None, height=None, textColor=(255, 255, 0), backgroundColor=None, justified='left')
        t.draw()
